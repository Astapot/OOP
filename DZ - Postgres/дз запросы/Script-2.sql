...
JOIN genre_perf gp ON mg.genre_id  = gp.genre_id 
GROUP BY mg.genre_id , mg.name
ORDER BY mg.genre_id  

-- ЗАДАНИЕ 3.2

SELECT name, COUNT(track_id) FROM albums a 
...
WHERE a.name IN (SELECT name FROM albums WHERE date_of_creation BETWEEN '2020-01-01' AND '2020-12-31')
